...
另:`AgentForeman.app` 是原生桌面应用(Swift + AppKit + WKWebView),两个可执行位于 `Contents/MacOS/`:`AgentForeman`(窗口程序,由 `AgentForemanApp.swift` 经 `build_app.sh` 用系统 swiftc 编译为 universal 二进制并提交)与 `launch-server.sh`(后端引导脚本)。启动流程 = 窗口程序先同步调用引导脚本(检测端口 → 未运行则 nohup 拉起 server.py) → 在 WKWebView 中加载 127.0.0.1:端口。引导脚本双模式:仓库模式直接用仓库里的 server.py;独立模式(内嵌项目到 Contents/Resources/app)先 rsync 到 `~/Library/Application Support/AgentForeman` 再运行(不带 --delete,保留用户数据)。端口可由 FOREMAN_PORT 覆盖(窗口程序、引导脚本、server.py 三者联动)。改窗口行为改 `.swift` 后需重跑 `build_app.sh` 重新编译;`dist/` 为构建产物已被 .gitignore。桌面版与网页版共享同一服务和数据,两者可并行使用;窗口菜单「显示 → 在浏览器中打开」(⌘⇧B)是从桌面版切到网页版的入口,改动服务端时需同时兼顾两种形态。
